...
class MotionDesigner(PlotterVispy):
    """
    A class for designing rational motions interactively.
    """
    ...
    def design(self,
        ...
        """
        Design a tool frame for a mechanism interactively.
        """
    ...
    def _design_tool_cubic_points(self):
        ...
        """
        from .MotionInterpolation import MotionInterpolation  # INNER IMPORT
        ...
        def update_curve(pts):
            ...
            # create curve using numpy Polynomials
            curve = [np.polynomial.Polynomial(c[::-1]) for c in coeffs]

            t_space = np.tan(np.linspace(-np.pi / 2, np.pi / 2, self.steps + 1))

            # evalute points
            curve_points = [DualQuaternion([c(t) for c in curve]) for t in t_space]
            return np.array([dq.dq2point_via_matrix() for dq in curve_points])

        curve_points = update_curve(points)
        plotted_curve = scene.visuals.Line(pos=curve_points,
                                           color='blue',
                                           width=2,
        ...
        # plot points
        markers = visuals.Markers()
        plotted_points = np.array([pt.normalized_in_3d() for pt in points])

        markers.set_data(plotted_points,
                         face_color='red',
                         size=10)
        self.view.add(markers)
        ...
        control_x = visuals.Markers()
        control_y = visuals.Markers()
        control_z = visuals.Markers()
        control_x.set_data(np.array([np.array([1, 0, 0])]),
                           face_color='red',
                           size=10)
        control_y.set_data(np.array([np.array([0, 1, 0])]),
                           face_color='green',
                           size=10)
        control_z.set_data(np.array([np.array([0, 0, 1])]),
                           face_color='blue',
                           size=10)
        self.view.add(control_x)
        self.view.add(control_y)
        self.view.add(control_z)
        ...
        def on_mouse_press(event):
            if event.button == 1:  # Left-click
                ...
                tr = markers.get_transform(map_to='canvas')
                marker_pos_2d = tr.map(plotted_points)
                # dehomogenize and left last coordinate
                marker_pos_2d /= marker_pos_2d[:, 3:]
                marker_pos_2d = marker_pos_2d[:, :2]

                # Calculate distances from mouse click to projected points
                click_pos = np.array(event.pos)
                distances = np.linalg.norm(marker_pos_2d - click_pos, axis=1)

                # Adjusted threshold for easier selection
                threshold = 30  # Pixels
                if np.min(distances) < threshold:
                    self.selected_idx = np.argmin(distances)

                    pt_origin = points[self.selected_idx].normalized_in_3d()
                    axis_pos = np.array([pt_origin,
                                         pt_origin + np.array([1, 0, 0]),
                                         pt_origin,
                                         pt_origin + np.array([0, 1, 0]),
                                         pt_origin,
                                         pt_origin + + np.array([0, 0, 1])])
                    frame = visuals.XYZAxis(pos=axis_pos)
                    self.view.add(frame)

                    control_x.set_data(np.array([pt_origin + np.array([1, 0, 0])]))
                    control_y.set_data(np.array([pt_origin + np.array([0, 1, 0])]))
                    control_z.set_data(np.array([pt_origin + np.array([0, 0, 1])]))
        ...
        def on_mouse_move(event):
            if event.is_dragging and self.selected_idx is not None:
                # Map 2D mouse position back to 3D world coordinates
                tr = self.view.scene.transform
                inv_tr = tr.inverse
                mouse_pos = event.pos
                new_pos = inv_tr.map([mouse_pos[0], mouse_pos[1], 0, 1])[:3]
                new_pos = [10, 2, 2]
                # Update control point position (keeping Z constant for simplicity)
                plotted_points[self.selected_idx][:2] = new_pos[:2]

                # Update markers and curve
                markers.set_data(plotted_points, face_color='red', size=10)
                new_curve_points = update_curve([PointHomogeneous.from_3d_point(p)
                                                 for p in plotted_points])
                plotted_curve.set_data(pos=new_curve_points, color='blue', width=2)
