...
class TestPlotter(unittest.TestCase):
    ...
    def test_plot_line(self):
        line = NormalizedLine()
        self.plt._plot_line = MagicMock()
        self.plt.plot(line)
        self.plt._plot_line.assert_called_once_with(line)

    def test_plot_point(self):
        point = PointHomogeneous()
        self.plt._plot_point = MagicMock()
        self.plt.plot(point)
        self.plt._plot_point.assert_called_once_with(point)

    def test_plot_dual_quaternion(self):
        dq = DualQuaternion()
        self.plt._plot_dual_quaternion = MagicMock()
        self.plt.plot(dq)
        self.plt._plot_dual_quaternion.assert_called_once_with(dq)

    def test_plot_transf_matrix(self):
        matrix = TransfMatrix()
        self.plt._plot_transf_matrix = MagicMock()
        self.plt.plot(matrix)
        self.plt._plot_transf_matrix.assert_called_once_with(matrix)

    def test_plot_rational_curve(self):
        mechanism = bennett_ark24()
        curve = mechanism.get_motion_curve()
        self.plt._plot_rational_curve = MagicMock()
        self.plt.plot(curve)
        self.plt._plot_rational_curve.assert_called_once_with(curve)

    def test_plot_interactive(self):
        mechanism = bennett_ark24()
        plot = Plotter(mechanism=mechanism, backend='matplotlib')
        t = 0.5
        plot.plot_slider_update(t, t_param=t)
    ...
    def test_plot_list(self):
        ...
        self.plt._plot_dual_quaternion = MagicMock()
        self.plt._plot_transf_matrix = MagicMock()
        self.plt.plot([dq1, dq2])
        self.plt._plot_dual_quaternion.assert_called_once_with(dq1)
        self.plt._plot_transf_matrix.assert_called_once_with(dq2)
